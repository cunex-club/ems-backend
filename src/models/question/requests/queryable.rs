...
impl Queryable for QueryableQuestion {
    fn to_where_clause<'sql>(self) -> SqlWhereClause<'sql> {
        ...
        .push_if_some(self.project_ids, |mut f, project_ids| {
            f.push_sql("election_id = ANY(SELECT id FROM elections WHERE project_id = ANY(")
        ...
        .push_if_some(self.owner_id, |mut f, owner_id| {
            f.push_sql("election_id = ANY(SELECT id FROM election WHERE project_id = ANY(SELECT id FROM projects WHERE owner_id = ")
        ...
        .push_if_some(self.member_ids, |mut f, member_ids| {
            f.push_sql(
                "(election_id = ANY(SELECT id FROM elections WHERE project_id = ANY(SELECT project_id FROM project_members WHERE user_id = ANY(",
            )
            .push_param(QueryParam::ArrayUuid(member_ids.clone()))
            .push_sql("))) OR election_id = ANY(SELECT id FROM elections WHERE project_id = ANY(SELECT id FROM projects WHERE owner_id = ANY(")
            .push_param(QueryParam::ArrayUuid(member_ids))
            .push_sql("))))");
            f
        })
        .push_if_some(self.election_ids, |mut f, election_ids| {
            f.push_sql("election_id = ANY(")
