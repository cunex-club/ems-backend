...
pub(crate) fn make_from(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl mysk_lib::models::traits::FetchLevelVariant<#db_variant> for #fetch_variant {
            async fn from_table(
                pool: &::sqlx::PgPool,
                table: #db_variant,
                _: Option<mysk_lib::common::requests::FetchLevel>,
                authorizer: &dyn mysk_lib::permissions::Authorizer,
            ) -> mysk_lib::prelude::Result<Self> {
